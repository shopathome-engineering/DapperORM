...
namespace ShopAtHome.DapperORMCore
{
    public static partial class SqlMapperExtensions
    {
        ...
        private static IEnumerable<PropertyInfo> ComputedPropertiesCache(Type type)
        {
            ...
            var computedProperties = TypePropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is NotMappedAttribute)).ToList();
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out sql))
            {
                ...
                sql = "select * from " + EncaseWithSquareBrackets(name) + " with (nolock) where " + onlyKey.Name + " = @id";
        ...
        internal static string EncaseWithSquareBrackets(string s)
        {
            const string openBracket = "[";
            const string closeBracket = "]";
            if (string.IsNullOrEmpty(s)) { return s; }
            return s.StartsWith(openBracket) && s.EndsWith(closeBracket) ? s 
                : s.StartsWith(openBracket) ? $"{s}{closeBracket}" 
                : s.EndsWith(closeBracket) ? $"{openBracket}{s}" 
                : $"{openBracket}{s}{closeBracket}";
        }
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!keyProperties.Any())
                throw new ArgumentException($"Entity must have at least one [Key] property. Entity is {type.FullName}");

            var name = GetTableName(type);

            var sb = new StringBuilder();
            sb.AppendFormat("update {0} with (rowlock) set ", name);
    ...
    {
        public void Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName,
            string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
        {
            var cmd = $"insert into {tableName} ({columnList}) values ({parameterList}); select SCOPE_IDENTITY() id";

            // At this time (June 8, 2016) Dapper doesn't allow the granularity necessary to identify multiple keys
            // on a table. Once that functionality is available, this block should be re-written to assign the key values
            // from the various SQL keys defined/identified by Dapper, rather than assuming the first identity column
            // returned from the SQL query is the key.
            var r = connection.Query(cmd, entityToInsert, transaction: transaction, commandTimeout: commandTimeout).FirstOrDefault();
            if(r != null && r.id != null)
            {
                var id = (int)r.id;
