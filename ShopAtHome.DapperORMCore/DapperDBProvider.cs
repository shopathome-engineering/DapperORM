...
namespace ShopAtHome.DapperORMCore
{
    ...
    public class DapperDBProvider : IDBProvider
    {
        ...
        {
            _mapper = DefaultMapper.Instance;
        }

        public DapperDBProvider(IMapper mapper)
        {
            _mapper = mapper;
        }
        ...
        {
            if (_mapper.ConfigurationProvider.FindTypeMapFor<dynamic, TData>() == null)
            {
            ...
            return connection.Query(storedProcedureName, param, commandType: CommandType.StoredProcedure, commandTimeout: timeoutSeconds).Select(_mapper.Map<TData>);
        }
        ...
        private IEnumerable<TData> ExecuteSQLQuery<TData>(IDbConnection connection, string sql, object param, int? timeoutSeconds = null)
        {
            try
            {
                ...
                return connection.Query(sql, param, commandType: CommandType.Text, commandTimeout: timeoutSeconds).Select(_mapper.Map<TData>);
            }
